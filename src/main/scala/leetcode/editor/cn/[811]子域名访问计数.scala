...
object Solution {
  def subdomainVisits(cpdomains: Array[String]): List[String] = {
    ...
    for (item <- cpdomains) {
      ...
      val times = strings(0)
      val address = strings(1)

      val strings1: Array[String] = address.split("\\.")

      strings1.reverse.foreach(addr => {
        val curAddr = if (pre == "") addr else addr + "." + pre
        pre = curAddr
        if (mymap.contains(curAddr)) mymap(curAddr) += times.toInt
        else mymap(curAddr) = times.toInt
      })

    }
    println(mymap)
    mymap.map(x => x._2 + " " + x._1).toList
  }
}
