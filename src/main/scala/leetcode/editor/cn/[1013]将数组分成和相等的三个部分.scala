...
object Solution {
  def canThreePartsEqualSum(A: Array[Int]): Boolean = {
    ...
    if (arr(A.length - 1) % 3 == 0) {
      ...
      println(onePart, twoPart)
      val slice = arr.slice(0, arr.length - 1)
      if (slice.contains(onePart) && slice.contains(twoPart)) {
        if (slice.indexWhere(_ == onePart) < slice.lastIndexWhere(_ == twoPart))
          return true
      }
    }
    return false
